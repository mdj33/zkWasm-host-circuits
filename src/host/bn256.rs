...
use std::ops::Shl;
use halo2_proofs::pairing::bn256::{G1, G2, G1Affine, G2Affine,
    Gt as Bn256Gt,
    Fq as Bn256Fq
};
...
fn bn256_g1_to_args(g:G1, op: BN256OP) -> Vec<ExternalHostCallEntry> {
    let mut a = bn256_fr_to_args(g.x, op);
    let mut b = bn256_fr_to_args(g.y, op);
...
fn bn256_g2_to_pair_args(g:G2) -> Vec<ExternalHostCallEntry> {
    let x0 = bn256_fr_to_args(g.x.c0, BN256OP::BN256PAIRG2);
    let x1 = bn256_fr_to_args(g.x.c1, BN256OP::BN256PAIRG2);
    let y0 = bn256_fr_to_args(g.y.c0, BN256OP::BN256PAIRG2);
    let y1 = bn256_fr_to_args(g.y.c1, BN256OP::BN256PAIRG2);
    let z:u64 = g.is_identity().unwrap_u8() as u64;
...
pub fn create_bn256_pair_shared_table(a: G1, b: G2) -> ExternalHostCallEntryTable {
...
pub fn create_bn256_sum_shared_table(ls: Vec<G1>, sum: G1) -> ExternalHostCallEntryTable {
    let mut r = ls.iter().map(|x| {
        bn256_g1_to_args(x.clone(), BN256OP::BN256ADD)
...
mod tests {
    use rand::rngs::OsRng;
    use crate::host::bn256::{
        create_bn256_pair_shared_table,
        create_bn256_sum_shared_table,
    };
    use std::fs::File;
    ...
    fn generate_bn256_pair_input() {
        ...
        let b = G2::random(&mut OsRng).into();
    ...
    fn generate_bn256_sum_input() {
        ...
        let ret = inputs[1..10].into_iter().fold(inputs[0], |acc:G1, x| {
